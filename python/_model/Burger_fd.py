...
class Burger_fd:
    ...
    def __init__(self, 
        ...
        if self.ssmforce == True and self.dforce == False:
            print("[Burger_fd] SSM forcing requires dforce")
            sys.exit()
        ...
        if M > 1:
            if kind == 'uniform':
                ...
                for i in range(self.M):
                    assert self.N % self.M == 0, "[Burger_fd] Something went wrong in basis setup"
                ...
                print("[Burger_fd] Basis function not known, exit..")
                sys.exit()
        else:
            self.basis = np.ones((self.M, self.N))
        ...
        if (v0 is None):
            if (u0 is None):
                    ...
                    # Turbulence
                        ...
                        print("[Burger_fd] Error: IC case unknown")
                        sys.exit()

            else:
                # check the input size
                if (np.size(u0,0) != self.N):
                    print("[Burger_fd] Error: wrong IC array size (is {}, expected {}".format(np.size(u0,0),self.N))
                    sys.exit()
            ...
            if (np.size(v0,0) != self.N):
                print("[Burger_fd] Error: wrong IC array size (is {}, expected {}".format(np.size(v0,0),self.N))
                sys.exit()
    ...
    def step( self, actions=None ):
        ...
        if (actions is not None):
            ...
            aforcing = np.matmul(actions, self.basis)
            self.actionHistory[self.ioutnum,:] = aforcing
        ...
        self.u += self.dt * (self.nu * d2udx2 - self.u * dudx + forcing)
        self.v = fft(self.u)
    ...
    def simulate(self, nsteps=None, restart=False, correction=[]):
        ...
        try:
            ...
            print("[Burger_fd] Floating point exception occured in simulate", flush=True)
            # something exploded
    ...
    def getMseReward(self, shift):
        ...
        try:
            ...
            print("[Burger_fd] Floating point exception occured in mse", flush=True)
    ...
    def getState(self, nAgents = None):
        ...
        try:
            ...
            # field and square
                ...
                print("[Burger_fd] Version not recognized", flush=True)
                sys.exit()


        except FloatingPointError:

            print("[Burger_fd] Floating point exception occured in getState", flush=True)
            if self.version == 0:
