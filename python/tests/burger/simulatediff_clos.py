...
sgs = Burger_clos(L=L, N=N2, dt=dt, nu=nu, tend=tEnd, ssm=False, dsm=True)
#u0 = f_IC(sgs.x)
#sgs.IC(u0 = u0)
...
errK_t = np.mean(((np.abs(dns.Ek_ktt[:,1:N2//2] - sgs.Ek_ktt[:,1:N2//2])/sgs.Ek_ktt[:,1:N2//2]))**2, axis=1)
