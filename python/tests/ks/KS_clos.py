...
class KS_clos:
    ...
    def __init__(self, L=2.*np.pi, N=512, dt=0.001, nu=1.0, dforce=True, nsteps=None, tend=5.0, u0=None, v0=None, case=None, noise=0., seed=42, ssm=False, dsm=False):
    ...
    #
    #     v1 = self.v + self.dt * (-0.5*self.k1*fft(self.u**2) + self.nu*(self.k2 - self.k4)*self.v + Fforcing)
    #     u1 = np.real(ifft(v1))
    #
    #     v2 = 3./4.*self.v + 1./4.*v1 + 1./4. * self.dt * (-0.5*self.k1*fft(u1**2) + self.nu*(self.k2 - self.k4)*v1 + Fforcing)
    #     u2 = np.real(ifft(v2))
    #
    #     v3 = 1./3.*self.v + 2./3.*v2 + 2./3. * self.dt * (-0.5*self.k1*fft(u2**2) + self.nu*(self.k2 - self.k4)*v2 + Fforcing)
    #     u3 = np.real(ifft(v3))
    #
    #     return (u3, v3)
    ...
    def step( self, C=None ):
        ...
        if self.ssm == True:
            ...
            sgs = 2*C*C*dx2*np.absolute(dudx)*d2udx2
            #sgs = 2*C*C*dx2*np.absolute(dudx)*d4udx4
            Fforcing += fft( sgs )
